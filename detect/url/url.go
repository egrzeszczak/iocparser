...
// URL represents a parsed URL with various formats.
type URL struct {
	Fanged   string
	Defanged string
	Original string
...
func init() {
	var err error
	// URLRegex is initialized with a regular expression pattern for detecting URLs.
	URLRegex, err = regexp2.Compile(`(?:(?:(?:https?|ftp):)?\/\/)(?:\S+(?::\S*)?@)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z0-9\u00a1-\uffff][a-z0-9\u00a1-\uffff_-]{0,62})?[a-z0-9\u00a1-\uffff]\.)+(?:[a-z\u00a1-\uffff]{2,}\.?))(?::\d{2,5})?(?:[/?#]\S*)?`, 0)
	if err != nil {
...
// Detect attempts to detect and parse a URL from the given input sample.
func Detect(sample string) (URL, error) {
	...
	// 2. Read the entire string and check if it's a URL (Link from start to finish).
	//
	// Run Full URL Regex check on the string.
